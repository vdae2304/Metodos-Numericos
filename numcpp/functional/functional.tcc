...
void resize(base_tensor<T, Rank, Tag> &arg, const shape_t<Rank> &shape) {
  if (arg.shape() != shape) {
    ...
    error << "non-broadcastable output operand with shape " << arg.shape()
...
inline base_tensor<detail::result_of_t<Function, T>, Rank,
...
inline base_tensor<detail::result_of_t<Function, T, U>, Rank,
...
apply(Function &&f, const base_tensor<T, Rank, TagT> &arg1,
  ...
  return base_tensor<Rt, Rank, Closure>(std::forward<Function>(f), arg1, arg2);
}
...
apply(Function &&f, const base_tensor<T, Rank, Tag> &arg1, const U &val) {
  ...
  return base_tensor<Rt, Rank, Closure>(std::forward<Function>(f), arg1, val);
}
...
apply(Function &&f, const T &val, const base_tensor<U, Rank, Tag> &arg2) {
  ...
  return base_tensor<Rt, Rank, Closure>(std::forward<Function>(f), val, arg2);
}
...
template <class Function, class T, size_t Rank, class Tag>
inline tensor<detail::result_of_t<
                  Function, base_tensor_const_axes_iterator<T, Rank, Tag, 1>,
                  base_tensor_const_axes_iterator<T, Rank, Tag, 1>>,
              Rank>
apply_along_axis(Function &&f, const base_tensor<T, Rank, Tag> &arg,
                 size_t axis) {
  typedef detail::result_of_t<Function,
                              base_tensor_const_axes_iterator<T, Rank, Tag, 1>,
                              base_tensor_const_axes_iterator<T, Rank, Tag, 1>>
      Rt;
  tensor<Rt, Rank> out;
...
inline tensor<detail::result_of_t<
                  Function, base_tensor_const_axes_iterator<T, Rank, Tag, N>,
 ...
 */
template <class F, typename = void> struct has_identity : std::false_type {};
...
reduce_impl(InputIterator first, InputIterator last, Function f,
            std::true_type) {
...
reduce(Function &&f, const base_tensor<T, Rank, Tag> &arg) {
  return detail::reduce_impl(arg.begin(), arg.end(), std::forward<Function>(f),
                             detail::has_identity<Function>());
}
...
template <class Function, class T, size_t Rank, class Tag>
inline tensor<typename base_tensor<T, Rank, Tag>::value_type, Rank>
accumulate(Function &&f, const base_tensor<T, Rank, Tag> &arg, size_t axis) {
  typedef typename base_tensor<T, Rank, Tag>::value_type Rt;
  tensor<Rt, Rank> out;
...
void accumulate(base_tensor<R, Rank, TagR> &out, Function &&f,
  ...
  shape[axis] = 1;
  for (index_t<Rank> out_index : make_index_sequence(shape)) {
    std::partial_sum(make_const_axes_iterator(&arg, out_index, axis, 0),
                     make_const_axes_iterator(&arg, out_index, axis, size),
...
template <class Function, class T, size_t M, class TagT, class U, size_t N,
          class TagU>
inline base_tensor<detail::result_of_t<Function, T, U>, M + N,
...
outer(Function &&f, const base_tensor<T, M, TagT> &arg1,
  ...
  return base_tensor<Rt, M + N, Closure>(std::forward<Function>(f), arg1, arg2);
}
...
void outer(base_tensor<R, M + N, TagR> &out, Function &&f,
  ...
  for (index_t<M> i : make_index_sequence_for(arg1)) {
    for (index_t<N> j : make_index_sequence_for(arg2)) {
      out[shape_cat(i, j)] = std::forward<Function>(f)(arg1[i], arg2[j]);
    }
  }
}
} // namespace numcpp
