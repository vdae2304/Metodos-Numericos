...
tensor_interface<T, Rank, Tag>::begin() {
  tensor_type &self = static_cast<tensor_type &>(*this);
  return iterator(&self, 0, self.layout());
}

template <class T, size_t Rank, class Tag>
inline base_tensor_const_iterator<T, Rank, Tag>
tensor_interface<T, Rank, Tag>::begin() const {
  const tensor_type &self = static_cast<const tensor_type &>(*this);
  return const_iterator(&self, 0, self.layout());
}
...
tensor_interface<T, Rank, Tag>::begin(layout_t order) const {
  const tensor_type &self = static_cast<const tensor_type &>(*this);
  return const_iterator(&self, 0, order);
}
...
tensor_interface<T, Rank, Tag>::end() {
  tensor_type &self = static_cast<tensor_type &>(*this);
  return iterator(&self, self.size(), self.layout());
}
...
tensor_interface<T, Rank, Tag>::end() const {
  const tensor_type &self = static_cast<const tensor_type &>(*this);
  return const_iterator(&self, self.size(), self.layout());
}
...
tensor_interface<T, Rank, Tag>::end(layout_t order) {
  tensor_type &self = static_cast<tensor_type &>(*this);
  return iterator(&self, self.size(), order);
}
...
tensor_interface<T, Rank, Tag>::end(layout_t order) const {
  const tensor_type &self = static_cast<const tensor_type &>(*this);
  return const_iterator(&self, self.size(), order);
}
...
tensor_interface<T, Rank, Tag>::apply_binary_function(
  ...
  shape_type common_shape = broadcast_shapes(shape, rhs.shape());
  ...
  for (index_type i : make_index_sequence_for(self)) {
...
base_tensor<T, Rank, Tag> &
tensor_interface<T, Rank, Tag>::apply_binary_function(Function f,
                                                      const T &val) {
  tensor_type &self = static_cast<tensor_type &>(*this);
  for (index_type i : make_index_sequence_for(self)) {
    self[i] = f(self[i], val);
...
inline base_tensor<T, Rank, Tag> &tensor_interface<T, Rank, Tag>::operator+=(
...
tensor_interface<T, Rank, Tag>::argpartition(size_t kth, Compare comp) const {
  const tensor_type &self = static_cast<const tensor_type &>(*this);
  index_sequence<Rank> indices(self.shape());
  tensor<index_type, 1> out(indices.begin(), self.size());
...
template <class Compare, detail::RequiresCallable<Compare, T, T>>
tensor<index_t<Rank>, 1>
tensor_interface<T, Rank, Tag>::argsort(Compare comp, bool stable) const {
  const tensor_type &self = static_cast<const tensor_type &>(*this);
  index_sequence<Rank> indices(self.shape());
  tensor<index_type, 1> out(indices.begin(), self.size());
  if (stable) {
...
template <class T, size_t Rank, class Tag>
inline void tensor_interface<T, Rank, Tag>::clamp(const T &a_min,
                                                  const T &a_max) {
...
tensor<index_t<Rank>, 1> tensor_interface<T, Rank, Tag>::nonzero() const {
  const tensor_type &self = static_cast<const tensor_type &>(*this);
  size_type size = self.size() - std::count(this->begin(), this->end(), T());
  tensor<index_type, 1> out(size);
  size_t n = 0;
  for (index_type i : make_index_sequence_for(self)) {
    if (self[i] != T()) {
      out[n++] = i;
...
template <class T, size_t Rank, class Tag>
inline void tensor_interface<T, Rank, Tag>::partition(size_t kth, size_t axis) {
  this->partition(kth, axis, less());
}

template <class T, size_t Rank, class Tag>
template <class Compare, detail::RequiresCallable<Compare, T, T>>
void tensor_interface<T, Rank, Tag>::partition(size_t kth, size_t axis,
                                               Compare comp) {
  tensor_type &self = static_cast<tensor_type &>(*this);
  shape_type shape = self.shape();
...
template <class T, size_t Rank, class Tag>
template <class Compare, detail::RequiresCallable<Compare, T, T>>
void tensor_interface<T, Rank, Tag>::sort(size_t axis, Compare comp,
                                          bool stable) {
  tensor_type &self = static_cast<tensor_type &>(*this);
  shape_type shape = self.shape();
  size_type size = shape[axis];
...
tensor<R, Rank>
tensor_interface<T, Rank, Tag>::apply_along_axis(Function f,
                                                 size_t axis) const {
  const tensor_type &self = static_cast<const tensor_type &>(*this);
  shape_type shape = self.shape();
  size_type size = shape[axis];
  ...
  for (index_type out_index : make_index_sequence(shape)) {
    out[out_index] = f(make_const_axes_iterator(&self, out_index, axis, 0),
...
tensor<R, Rank>
tensor_interface<T, Rank, Tag>::apply_over_axes(Function f,
                                                const shape_t<N> &axes) const {
  static_assert(N <= Rank, "Cannot apply reduction over more axes than the "
                           "tensor dimension");
  const tensor_type &self = static_cast<const tensor_type &>(*this);
  shape_type shape = self.shape();
  size_type size = 1;
  ...
  tensor<R, Rank> out(shape);
  for (index_type out_index : make_index_sequence(shape)) {
    out[out_index] = f(make_const_axes_iterator(&self, out_index, axes, 0),
...
inline index_t<Rank> tensor_interface<T, Rank, Tag>::argmax() const {
  const tensor_type &self = static_cast<const tensor_type &>(*this);
  ranges::argmax pred;
  size_type index = pred(this->begin(), this->end());
...
tensor_interface<T, Rank, Tag>::argmax(size_t axis) const {
  return this->apply_along_axis<size_type>(ranges::argmax(), axis);
}
...
tensor_interface<T, Rank, Tag>::argmin(size_t axis) const {
  return this->apply_along_axis<size_type>(ranges::argmin(), axis);
}
...
template <size_t N>
inline tensor<typename std::remove_cv<T>::type, Rank>
tensor_interface<T, Rank, Tag>::stddev(const shape_t<N> &axes,
                                       bool bias) const {
  typedef typename std::remove_cv<T>::type Rt;
...
template <size_t N>
inline tensor<typename std::remove_cv<T>::type, Rank>
tensor_interface<T, Rank, Tag>::var(const shape_t<N> &axes, bool bias) const {
  typedef typename std::remove_cv<T>::type Rt;
...
inline base_tensor<T, Rank,
                   lazy_unary_tag<math::real, const std::complex<T>, Tag>>
...
void complex_interface<std::complex<T>, Rank, Tag>::real(
  ...
  if (self.shape() != arg.shape()) {
    std::ostringstream error;
    error << "input shape " << arg.shape() << " doesn't match the output shape "
          << self.shape();
    throw std::invalid_argument(error.str());
  }
  for (index_type i : make_index_sequence_for(self)) {
...
template <class T, size_t Rank, class Tag>
inline base_tensor<T, Rank,
                   lazy_unary_tag<math::imag, const std::complex<T>, Tag>>
