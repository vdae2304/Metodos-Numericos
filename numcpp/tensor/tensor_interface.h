 ...
 * subclasses. A tensor_interface works by using the Curiously Recurring
 * Template Pattern @link https://en.cppreference.com/w/cpp/language/crtp
 * @endlink
 *
...
public:
  ...
  base_tensor<T, Rank, Tag> &operator+=(const base_tensor<T, Rank, TagOp> &rhs);
  template <class TagOp>
  base_tensor<T, Rank, Tag> &operator-=(const base_tensor<T, Rank, TagOp> &rhs);
  template <class TagOp>
  base_tensor<T, Rank, Tag> &operator*=(const base_tensor<T, Rank, TagOp> &rhs);
  ...
  operator<<=(const base_tensor<T, Rank, TagOp> &rhs);
  template <class TagOp>
  base_tensor<T, Rank, Tag> &
  operator>>=(const base_tensor<T, Rank, TagOp> &rhs);
  ...
  /**
   * @brief Test whether all tensor elements over the given axes evaluate to
   * true.
   *
   ...
   *         output tensor has the same dimension as @a *this, but the axes
  ...
  template <size_t N> tensor<bool, Rank> all(const shape_t<N> &axes) const;
  ...
  /**
   * @brief Test whether any tensor element over the given axes evaluate to
   * true.
   *
  ...
  template <size_t N> tensor<bool, Rank> any(const shape_t<N> &axes) const;
   ...
   *         axis. The output tensor has the same dimension as @a *this, but the
   *         axis which is reduced is left as a dimension of size one.
   *
   * @throw std::bad_alloc If the function fails to allocate storage it may
   *                       throw an exception.
   */
 ...
 *           @c a[indices] yields a partitioned @a a.
   ...
   *
   * @return A 1-dimensional tensor of indices that sort the tensor. If
   *         @a indices is the returned tensor of indices for tensor @a a, then
   *         @c a[indices] yields a sorted @a a.
   *
   * @throw std::bad_alloc If the function fails to allocate storage it may
   *                       throw an exception.
   */
   ...
   */
  template <class U>
  base_tensor<U, Rank, lazy_unary_tag<cast_to<T, U>, T, Tag>> astype() const;
   ...
   *         tensor has the same dimension as @a *this, but the axes which are
   *         reduced are left as dimensions of size one.
   *
   * @throw std::bad_alloc If the function fails to allocate storage it may
   *                       throw an exception.
   */
  tensor<typename std::remove_cv<T>::type, Rank> max(size_t axis) const;
   ...
   *             computed.
   *
   * @return A new tensor with the average over the given axes. The output
   *         tensor has the same dimension as @a *this, but the axes which are
   *         reduced are left as dimensions of size one.
   *
   * @throw std::bad_alloc If the function fails to allocate storage it may
   *                       throw an exception.
   */
  tensor<typename std::remove_cv<T>::type, Rank> mean(size_t axis) const;
   ...
   *                       throw an exception.
   */
  tensor<typename std::remove_cv<T>::type, Rank> min(size_t axis) const;
   ...
   *             performed.
   *
   * @return A new tensor with the product over the given axes. The output
   *         tensor has the same dimension as @a *this, but the axes which are
   *         reduced are left as dimensions of size one.
   *
   * @throw std::bad_alloc If the function fails to allocate storage it may
   *                       throw an exception.
   */
  tensor<typename std::remove_cv<T>::type, Rank> prod(size_t axis) const;
   ...
   *                       throw an exception.
   */
  tensor<typename std::remove_cv<T>::type, Rank> stddev(size_t axis,
                                                        bool bias) const;

  template <size_t N>
  tensor<typename std::remove_cv<T>::type, Rank> stddev(const shape_t<N> &axes,
                                                        bool bias) const;
   ...
   *             performed.
   *
   * @return A new tensor with the sum over the given axes. The output tensor
   *         has the same dimension as @a *this, but the axes which are reduced
   *         are left as dimensions of size one.
   *
   * @throw std::bad_alloc If the function fails to allocate storage it may
   *                       throw an exception.
   */
  tensor<typename std::remove_cv<T>::type, Rank> sum(size_t axis) const;
   ...
   *             normalization is by @a n - 1.
   *
   * @return A new tensor with the variance over the given axes. The output
   *         tensor has the same dimension as @a *this, but the axes which are
   *         reduced are left as dimensions of size one.
   *
   * @throw std::bad_alloc If the function fails to allocate storage it may
   *                       throw an exception.
   */
  tensor<typename std::remove_cv<T>::type, Rank> var(size_t axis,
                                                     bool bias) const;

  template <size_t N>
  tensor<typename std::remove_cv<T>::type, Rank> var(const shape_t<N> &axes,
                                                     bool bias) const;
...
private:
  ...
  typedef base_tensor<T, Rank, Tag> tensor_type;

  /**
   * @brief Helper function. Apply a binary function with another tensor or with
   * a value.
   */
  template <class Function, class TagOp>
  base_tensor<T, Rank, Tag> &
  apply_binary_function(Function f, const base_tensor<T, Rank, TagOp> &rhs);

  template <class Function>
  base_tensor<T, Rank, Tag> &apply_binary_function(Function f, const T &val);
  ...
  tensor<R, Rank> apply_over_axes(Function f, const shape_t<N> &axes) const;
};
 ...
 * tensor_interface. A complex_interface works by using the Curiously Recurring
 * Template Pattern @link https://en.cppreference.com/w/cpp/language/crtp
 * @endlink
 *
...
public:
   ...
   */
  template <class TagReal> void real(const base_tensor<T, Rank, TagReal> &arg);
   ...
   * @throw std::invalid_argument Thrown if the shapes are different.
   */
  template <class TagImag> void imag(const base_tensor<T, Rank, TagImag> &arg);
...
private:
  ...
  typedef shape_t<Rank> shape_type;
  typedef index_t<Rank> index_type;
  typedef base_tensor<std::complex<T>, Rank, Tag> tensor_type;
};

/**
 * @brief Partial specialization of complex_interface for const std::complex.
 */
template <class T, size_t Rank, class Tag>
...
public:
  ...
  base_tensor<T, Rank, lazy_unary_tag<math::real, const std::complex<T>, Tag>>
  real() const;

  base_tensor<T, Rank, lazy_unary_tag<math::imag, const std::complex<T>, Tag>>
  imag() const;
...
private:
  ...
  typedef shape_t<Rank> shape_type;
  typedef index_t<Rank> index_type;
  typedef base_tensor<const std::complex<T>, Rank, Tag> tensor_type;
};
} // namespace numcpp
