 ...
 */
template <class T> class base_tensor<T, 1, sequence_tag> {
public:
  /// Member types.
...
public:
  ...
  base_tensor(const T &start, size_type size, const T &step, const T &base)
      : m_start(start), m_size(size), m_step(step), m_base(new T(base)) {}

  /// Destructor.
  ~base_tensor() { delete m_base; };
   ...
   * sequence. It does not point to any element, and thus shall not be
   * dereferenced.
   *
   ...
   *
   * @param i An @c index_t object with the position of an element in the
   *          sequence.
   *
   * @return The element at the specified position.
   */
