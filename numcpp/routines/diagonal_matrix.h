...
namespace numcpp {
    ...
    public:
        ...
        T operator()(size_t i, size_t j) const {
            __assert_within_bounds(m_size, m_size, i, j);
            if (m_offset >= 0) {
                return (i + m_offset == j) ? m_arr[i] : T(0);
            }
            else {
                return (i == j - m_offset) ? m_arr[j] : T(0);
            }
        }

        /**
         * @brief Subscript operator. Returns the element at a given row and
         * column in the diagonal_matrix.
         *
         * @param index An index_t object with the row and column position of
         *     an element in the diagonal_matrix.
         *
         * @return The element at the specified row and column in the
         *     diagonal_matrix.
         *
    ...
    public:
         ...
         : m_mat(mat), m_offset(offset) {
            ...
            if (offset >= 0) {
                if (mat.cols() > (size_t)offset) {
                    m_size = std::min(mat.rows(), mat.cols() - offset);
                }
            }
            else {
                if (mat.rows() > (size_t)-offset) {
                    m_size = std::min(mat.rows() + offset, mat.cols());
                }
            }
        }
        ...
        T operator[](size_t i) const {
            ...
            if (m_offset >= 0) {
                return m_mat(i, i + m_offset);
            }
            else {
                return m_mat(i - m_offset, i);
            }
        }
    ...
    public:
        ...
        T operator()(size_t i, size_t j) const {
            ...
            if (m_offset >= 0) {
                return (i + m_offset == j) ? T(1) : T(0);
            }
            else {
                return (i == j - m_offset) ? T(1) : T(0);
            }
        }

        /**
         * @brief Subscript operator. Returns the element at a given row and
         * column in the eye_matrix.
         *
         * @param index An index_t object with the row and column position of
         *     an element in the eye_matrix.
         *
         * @return The element at the specified row and column in the
         *     eye_matrix.
         *
