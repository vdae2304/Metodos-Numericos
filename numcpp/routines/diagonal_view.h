...
struct eye_tag;
template <class Tag> struct diagonal_tag;
template <class Tag> struct triangular_tag;

/**
 * @brief A light-weight object with ones on the diagonal and zeros elsewhere.
 * This class represents a 2-dimensional expression.
 *
 * @tparam T Type of the elements contained in the tensor.
 */
template <class T> class base_tensor<T, 2, eye_tag> {
public:
  /// Member types.
...
public:
   ...
   */
  static constexpr size_type ndim() { return 2; }
   ...
   */
  tensor<value_type, 2> copy() const { return tensor<value_type, 2>(*this); }
};

/**
 * @brief A light-weight object with its elements extracted from the diagonal
 * of a matrix. This class represents a view of the elements of another tensor
 * rather than a new tensor.
 *
 * @tparam T Type of the elements contained in the tensor.
 * @tparam Tag Type of the base_tensor container to take the diagonal from.
 */
template <class T, class Tag> class base_tensor<T, 1, diagonal_tag<Tag>> {
public:
  /// Member types.
  ...
  typedef nullptr_t pointer;
  typedef nullptr_t const_pointer;
  typedef base_tensor_const_iterator<T, 1, diagonal_tag<Tag>> iterator;
  typedef base_tensor_const_iterator<T, 1, diagonal_tag<Tag>> const_iterator;
  typedef size_t size_type;
  typedef ptrdiff_t difference_type;
...
public:
  ...
  /**
   * @brief Constructs an array with its elements extracted from the diagonal of
   * a matrix.
   *
   ...
   * @return A random access iterator to the beginning of the tensor.
   */
  const_iterator begin(layout_t = default_layout) const {
   ...
   * @return A random access iterator to the element past the end of the tensor.
   */
  const_iterator end(layout_t = default_layout) const {
   ...
   * @brief Return the dimension of the tensor.
   */
  static constexpr size_type ndim() { return 1; }
   ...
   *             axes.
   */
  shape_type shape() const { return shape_type(m_size); }

  size_type shape(size_type axis) const { return (axis == 0) ? m_size : 0; }
   ...
   * @brief Return a copy of the tensor.
   */
  tensor<value_type, 1> copy() const { return tensor<value_type, 1>(*this); }
};

/**
 * @brief A light-weight object with given values on the diagonal and zeros
 * elsewhere. This class represents a view of the elements of another tensor
 * rather than a new tensor.
 *
 * @tparam T Type of the elements contained in the tensor.
 * @tparam Tag Type of the base_tensor container with the elements on the
 *             diagonal.
 */
template <class T, class Tag> class base_tensor<T, 2, diagonal_tag<Tag>> {
public:
  /// Member types.
  typedef typename std::remove_cv<T>::type value_type;
  typedef T reference;
  ...
  typedef nullptr_t pointer;
  typedef nullptr_t const_pointer;
  typedef base_tensor_const_iterator<T, 2, diagonal_tag<Tag>> iterator;
  typedef base_tensor_const_iterator<T, 2, diagonal_tag<Tag>> const_iterator;
  typedef size_t size_type;
  typedef ptrdiff_t difference_type;
  typedef shape_t<2> shape_type;
  typedef index_t<2> index_type;
 ...
 *
 * @tparam T Type of the elements contained in the tensor.
 * @tparam Tag Type of the base_tensor container with the elements below/above
 *             the diagonal.
 */
template <class T, class Tag> class base_tensor<T, 2, triangular_tag<Tag>> {
public:
  /// Member types.
  typedef typename std::remove_cv<T>::type value_type;
  typedef T reference;
  ...
  typedef nullptr_t pointer;
  typedef nullptr_t const_pointer;
  typedef base_tensor_const_iterator<T, 2, triangular_tag<Tag>> iterator;
  typedef base_tensor_const_iterator<T, 2, triangular_tag<Tag>> const_iterator;
  typedef size_t size_type;
  typedef ptrdiff_t difference_type;
  typedef shape_t<2> shape_type;
  typedef index_t<2> index_type;
...
public:
   ...
   * @param k Offset of the diagonal from the main diagonal
   */
  base_tensor(const base_tensor<T, 2, Tag> &a, bool lower = true,
              difference_type k = 0)
   ...
   *             axes.
   */
  auto shape() -> decltype(m_arg.shape()) { return m_arg.shape(); }
