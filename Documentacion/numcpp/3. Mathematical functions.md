...
that x = significand * 2<sup>exponent</sup>, element-wise.
```cpp
template <class T>
array< std::pair<T, int> > frexp(const array<T> &x);
```
...
```cpp
template <class T>
matrix< std::pair<T, int> > frexp(const matrix<T> &x);
```
...
```
[(0, 0), (0.5, 1), (0.5, 2), (0.75, 2), (0.5, 3), (0.625, 3), (0.75, 3),
(0.875, 3), (0.5, 4), (0.5625, 4)]
```
