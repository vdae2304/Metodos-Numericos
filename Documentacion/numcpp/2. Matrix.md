...
matrix(InputIterator first, InputIterator last, size_t n);
```
...
elements specified by the array of pairs of indices. 
```cpp
index_view<T> operator[](const array< std::pair<size_t, size_t> > &index);
array<T> operator[](const array< std::pair<size_t, size_t> > &index) const;
```

Parameters

* `index` An `array< std::pair<size_t, size_t> >` object with its elements 
identifying which elements of the matrix are selected.
...
int main() {
    np::matrix<int> mat{{10, -1, 5, 3}, 
                        {7, 17, 11, 6}, 
                        {8, -5, 1, -11}};
    np::array< std::pair<size_t, size_t> > index{{0, 0}, {0, 2}, {1, 3}, {2, 1}};
...
* A light-weight object with the elements in the `matrix` casted  to the specified type. Convertible to a `matrix` object.
