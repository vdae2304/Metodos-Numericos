...
namespace scicpp {
    ...
    LinAlgError::LinAlgError(const std::string &what_arg) {
        this->what_arg = what_arg;
    }

    const char* LinAlgError::what() const throw() {
    ...
    template <class T>
    T norm(const numcpp::array<T> &v, double p) {
        T out = T(0);
        if (p < 1) {
    ...
    template <class T>
    numcpp::matrix<T> pinv(const numcpp::matrix<T> &A, const T &cond) {
        return lstsq(A, numcpp::eye<T>(A.rows(), A.rows()), "svd", cond);
    }

    // Constructs an orthonormal basis for the range of A using SVD.
    template <class T>
    numcpp::matrix<T> orth(const numcpp::matrix<T> &A, const T &cond) {
        numcpp::matrix<T> U, V;
        numcpp::array<T> S;
        svd(A, U, S, V, false);
    ...
    template <class T>
    numcpp::matrix<T> null_space(const numcpp::matrix<T> &A, const T &cond) {
        numcpp::matrix<T> U, V;
        numcpp::array<T> S;
        svd(A, U, S, V, true);
